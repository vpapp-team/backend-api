exports.onRequest = (req, resp, subpath, location) => {
  ...
  process.util.parseDataStatus(req, (err, parsedDataStatus) => {
    ...
    console.log('---Stand-ins.js requestData', requestData);

    process.util.requestMaster('api/v1.0.0/Stand-ins', data => {
      console.log('---Stand-ins.js from Master', JSON.stringify({ data }));
      ...
      console.log('---Stand-ins.js add/remov', JSON.stringify({ added, removed }));
      ...
      process.util.accept(resp, {
        ...
        add: added.map(a => new (process.types.get('Data').get('Stand-in'))(a).simplify()),
...
exports.handleData = range => new Promise(resolve => {
  console.log('reached Stand-ins.js handleData');
  console.log('versionRequest StandIns');
  ...
  if (bufferedStandInsAt < lastUpdate.lastUpdate.rawNumber) {
    process.util.promisifiedQuery(process.sqlPool, `SELECT * FROM ${process.config.mysql_readwrite.tables.STANDINS}
      WHERE (
        (added BETWEEN ? AND ?)
      OR
        ((outdated IS NOT NULL) AND (outdated BETWEEN ? AND ?))
      )`, [`DT${bufferedStandInsAt}`, `DT${lastUpdate.lastUpdate.rawNumber}`, `DT${bufferedStandInsAt}`, `DT${lastUpdate.lastUpdate.rawNumber}`]).then(rows => {
      ...
      console.error('handleData StandIns', err);
      resolve(returnDaysInRange(new (process.types.get('General').get('Range'))(range)));
    });
  } else {
    resolve(returnDaysInRange(new (process.types.get('General').get('Range'))(range)));
