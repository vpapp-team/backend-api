...
// BufferedVersions = [type:Endpoints.toSQL()]
...
const EndpointsMayHaveChanged = () => {
  ...
  UTIL.promisifiedQuery(CONFIG.sqlPool, `SELECT * FROM ${CONFIG.mysql_read.tables.ENDPOINTS}`).then(rows => {
    ...
    if (bufferedVersions.length && !WORKERS.length) {
      LOGGER.log(`pulled ${bufferedVersions.length} versions - launching workers`);
      startWorkers();
    } else {
...
const startWorkers = () => {
  // Workers get started when #EndpointsMayHaveChanged successfully pulls endpoints
...
const registerAtProxy = () => {
  ...
  const request = (CONFIG.proxy.secure ? HTTPS : HTTP).request({
...
const onDataChange = () => {
  ...
  UTIL.promisifiedQuery(CONFIG.sqlPool, `SELECT * FROM ${CONFIG.mysql_read.tables.UPDATES}`).then(rows => {
    ...
    LOGGER.error('onDataChange failed to select from sql', err);
  });
};
onDataChange();
